...
class TestLedgerCreation:
    ...
    def test_create_with_initial_time(self):
        ...
        ledger = Ledger("test", initial_time=t, verbose=False, test_mode=True)
...
class TestUnitRegistration:
    ...
    def test_update_unit_state(self):
        ledger = Ledger("test", verbose=False, test_mode=True)
        ledger.register_unit(_stock("AAPL", "Apple", "AAPL_ISSUER"))
        ledger.update_unit_state("AAPL", {"price": Decimal("150")})
        state = ledger.get_unit_state("AAPL")
        assert state["price"] == Decimal("150")
...
class TestBalanceOperations:
    ...
    def test_get_wallet_balances(self):
        ...
        ledger.register_wallet("alice")
        ledger.balances["alice"]["USD"] = Decimal("1000.0")
        ledger.balances["alice"]["AAPL"] = Decimal("10.0")

        bals = ledger.get_wallet_balances("alice")
        assert bals["USD"] == Decimal("1000")
        assert bals["AAPL"] == Decimal("10")
    ...
    def test_total_supply(self):
        ...
        ledger.balances["bob"]["USD"] = Decimal("-500")

        assert ledger.total_supply("USD") == Decimal("500")
...
class TestTransactionExecution:
    ...
    def test_execute_multi_move_transaction(self):
        ...
        ledger.balances["alice"]["USD"] = Decimal("10000.0")
        ledger.balances["bob"]["AAPL"] = Decimal("100.0")
...
class TestExecuteContract:
    ...
    def test_execute_with_state_changes(self):
        ...
        new_state = {**old_state, "settled": True, "price": Decimal("150")}
...
class TestCloneAndReplay:
    ...
    def test_clone_creates_independent_copy(self):
        ...
        # Modify clone
        clone.balances["alice"]["USD"] = Decimal("500.0")
    ...
    def test_clone_copies_units(self):
        ...
        # Modify clone state
        clone.update_unit_state("AAPL", {"price": Decimal("200")})

        # Original unchanged
        assert ledger.get_unit_state("AAPL")["price"] == Decimal("150")
        assert clone.get_unit_state("AAPL")["price"] == Decimal("200")
    ...
    def test_replay_reconstructs_state(self):
        ...
        assert replayed.get_balance("alice", "USD") == Decimal("700.0")
        assert replayed.get_balance("bob", "USD") == Decimal("300.0")
        assert replayed.get_balance("treasury", "USD") == Decimal("-1000.0")
...
class TestStateVerification:
    ...
    def test_ledger_state_equals_different(self):
        ...
        ledger.advance_time(datetime(2025, 1, 2))
        tx = build_transaction(ledger, [Move(Decimal("100.0"), "USD", "alice", "bob", "p")])
        ledger.execute(tx)
    ...
    def test_compare_ledger_states_details(self):
        ...
        ledger1.balances["alice"]["USD"] = Decimal("1000.0")
        ...
        ledger2.balances["alice"]["USD"] = Decimal("900.0")
