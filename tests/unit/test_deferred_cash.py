...
class TestCreateDeferredCashUnit:
    ...
    def test_create_deferred_cash_unit_basic(self):
        ...
        assert unit.state['amount'] == 15000.0
        assert unit.state['currency'] == "USD"
        assert unit.state['payment_date'] == datetime(2024, 3, 17)
        assert unit.state['payer_wallet'] == "buyer"
        assert unit.state['payee_wallet'] == "seller"
        assert unit.state['settled'] is False
        assert unit.state['reference'] is None
    ...
    def test_create_deferred_cash_unit_with_reference(self):
        ...
        assert unit.state['reference'] == "trade_456"
    ...
    def test_create_deferred_cash_unit_quantity_always_one(self):
        ...
        assert unit.max_balance == Decimal("1.0")
        assert unit.decimal_places == Decimal("0")  # No fractional units
...
class TestComputeDeferredCashSettlement:
    ...
    def setup_ledger(self):
        ...
        ledger = Ledger("test", datetime(2024, 3, 15), verbose=False, test_mode=True)
        ...
        ledger.set_balance("buyer", "USD", Decimal("50000.0"))
    ...
    def test_settlement_on_payment_date(self, setup_ledger):
        ...
        ledger.set_balance("buyer", "DC_trade_123", Decimal("1"))
        ...
        assert cash_move.quantity == Decimal("15000.0")
        ...
        assert extinguish_move.quantity == Decimal("1.0")
    ...
    def test_settlement_updates_ledger_balances(self, setup_ledger):
        ...
        assert ledger.get_balance("buyer", "USD") == Decimal("50000.0")
        assert ledger.get_balance("seller", "USD") == Decimal("0.0")
        assert ledger.get_balance("buyer", "DC_trade_123") == Decimal("1.0")
        ...
        assert ledger.get_balance("buyer", "USD") == Decimal("35000.0")  # -15000
        assert ledger.get_balance("seller", "USD") == Decimal("15000.0")  # +15000
        assert ledger.get_balance("buyer", "DC_trade_123") == Decimal("0.0")  # Extinguished
        assert ledger.get_balance(SYSTEM_WALLET, "DC_trade_123") == Decimal("1.0")  # Returned to system
...
class TestDeferredCashContractIntegration:
    ...
    def test_lifecycle_engine_processes_settlement(self):
        ...
        assert ledger.get_balance("buyer", "USD") == Decimal("75000.0")  # -25000
        assert ledger.get_balance("seller", "USD") == Decimal("25000.0")  # +25000
        assert ledger.get_balance("buyer", "DC_trade_123") == Decimal("0.0")  # Extinguished
    ...
    def test_multiple_deferred_cash_different_dates(self):
        ...
        ledger = Ledger("test", datetime(2024, 3, 1), verbose=False, test_mode=True)
        ledger.register_unit(cash("USD", "US Dollar"))
        ...
        ledger.set_balance("buyer", "DC_trade_1", Decimal("1"))
        ledger.set_balance("buyer", "DC_trade_2", Decimal("1"))
        ledger.set_balance("buyer", "DC_trade_3", Decimal("1"))
        ...
        assert ledger.get_balance("buyer", "USD") == Decimal("55000.0")  # -45000 total
        assert ledger.get_balance("seller", "USD") == Decimal("45000.0")  # +45000 total
        assert ledger.get_balance("buyer", "DC_trade_1") == Decimal("0.0")
        assert ledger.get_balance("buyer", "DC_trade_2") == Decimal("0.0")
        assert ledger.get_balance("buyer", "DC_trade_3") == Decimal("0.0")
...
class TestConservationLaws:
    ...
    def test_conservation_cash_throughout_lifecycle(self):
        ...
        # Initial USD supply
        ledger.set_balance("buyer", "USD", Decimal("100000.0"))
        ledger.set_balance("seller", "USD", Decimal("50000.0"))
        ...
        # Verify balances
        assert ledger.get_balance("buyer", "USD") == Decimal("70000.0")
        assert ledger.get_balance("seller", "USD") == Decimal("80000.0")
    ...
    def test_conservation_deferred_cash_lifecycle(self):
        ...
        assert ledger.total_supply("DC_trade_123") == Decimal("0.0")
        ...
        assert ledger.get_balance("buyer", "DC_trade_123") == Decimal("0.0")
        assert ledger.get_balance(SYSTEM_WALLET, "DC_trade_123") == Decimal("1.0")  # Returned to system
...
class TestT2SettlementPattern:
    ...
    def test_t2_stock_trade_complete_flow(self):
        ...
        # Initial positions
        ledger.set_balance("buyer", "USD", Decimal("100000.0"))
        ledger.set_balance("seller", "AAPL", Decimal("100.0"))
        ...
        ledger.set_balance("seller", "AAPL", Decimal("0.0"))  # Seller no longer has stock
        ...
        ledger.register_unit(dc_unit)
        ledger.set_balance("buyer", "DC_trade_AAPL_20240315", Decimal("1"))

        # After trade date: buyer has stock, obligation; seller has neither
        assert ledger.get_balance("buyer", "AAPL") == Decimal("100.0")
        assert ledger.get_balance("seller", "AAPL") == Decimal("0.0")
        assert ledger.get_balance("buyer", "DC_trade_AAPL_20240315") == Decimal("1.0")
        assert ledger.get_balance("buyer", "USD") == Decimal("100000.0")  # Cash not yet moved
        assert ledger.get_balance("seller", "USD") == Decimal("0.0")
        ...
        assert ledger.get_balance("buyer", "USD") == Decimal("85000.0")  # -15000
        assert ledger.get_balance("seller", "USD") == Decimal("15000.0")  # +15000
        assert ledger.get_balance("buyer", "DC_trade_AAPL_20240315") == Decimal("0.0")  # Extinguished
    ...
    def test_t2_settlement_with_lifecycle_engine(self):
        ...
        ledger.set_balance("buyer", "USD", Decimal("200000.0"))
        ledger.set_balance("seller", "AAPL", Decimal("100.0"))
        ...
        ledger.set_balance("buyer", "AAPL", Decimal("100.0"))
        ...
        ledger.register_unit(dc_unit)
        ledger.set_balance("buyer", "DC_AAPL_T2", Decimal("1"))
        ...
        assert ledger.get_balance("buyer", "USD") == Decimal("185000.0")
        assert ledger.get_balance("seller", "USD") == Decimal("15000.0")
...
class TestDividendDeferredCashPattern:
    ...
    def test_dividend_deferred_cash_creation(self):
        ...
        ledger.set_balance("alice", dc_unit.symbol, Decimal("1"))

        # Alice has dividend entitlement, but cash not yet paid
        assert ledger.get_balance("alice", dc_unit.symbol) == Decimal("1.0")
        assert ledger.get_balance("alice", "USD") == Decimal("0.0")
        ...
        assert ledger.get_balance("alice", dc_unit.symbol) == Decimal("0.0")
    ...
    def test_dividend_position_change_after_ex_date(self):
        ...
        ledger.set_balance("treasury", "USD", Decimal("1000000.0"))
        ledger.set_balance("alice", "AAPL", Decimal("1000.0"))
        ...
        ledger.set_balance("alice", "AAPL", Decimal("0.0"))  # Alice no longer has stock
        ledger.set_balance("bob", "AAPL", Decimal("1000.0"))
        ...
        assert ledger.get_balance("alice", "USD") == Decimal("500.0")
        assert ledger.get_balance("bob", "USD") == Decimal("0.0")  # Bob gets nothing
