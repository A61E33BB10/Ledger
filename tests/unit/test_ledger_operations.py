...
class TestUnitRegistration:
    ...
    def test_update_unit_state(self):
        ...
        ledger = Ledger("test", verbose=False, test_mode=True)
        ledger.register_unit(_stock("AAPL", "Apple", "AAPL_ISSUER"))
        ledger.update_unit_state("AAPL", {"price": Decimal("150.0"), "custom": "value"})
        state = ledger.get_unit_state("AAPL")
...
class TestBalanceOperations:
    ...
    def test_get_wallet_balances(self):
        ...
        ledger.register_wallet("alice")
        ledger.set_balance("alice", "USD", Decimal("1000.0"))
        ledger.set_balance("alice", "AAPL", Decimal("10.0"))
    ...
    def test_get_positions(self):
        ...
        ledger.set_balance("alice", "AAPL", Decimal("100.0"))
        ledger.set_balance("bob", "AAPL", Decimal("50.0"))
        # charlie has zero - should not appear

        positions = ledger.get_positions("AAPL")
        assert positions == {"alice": Decimal("100.0"), "bob": Decimal("50.0")}
    ...
    def test_total_supply(self):
        ...
        ledger.set_balance("alice", "USD", Decimal("1000.0"))
        ledger.set_balance("bob", "USD", Decimal("500.0"))

        assert ledger.total_supply("USD") == Decimal("1500.0")
    ...
    def test_total_supply_with_negative(self):
        ...
        assert ledger.total_supply("USD") == Decimal("500.0")
...
class TestTimeManagement:
    ...
    def test_advance_time_same_time_ok(self):
        ...
        t = datetime(2025, 1, 1)
        ledger = Ledger("test", t, verbose=False, test_mode=True)
        ledger.advance_time(t)  # Should not raise
        assert ledger.current_time == t

    def test_advance_time_backwards_raises(self):
        """advance_time backwards raises."""
        ledger = Ledger("test", datetime(2025, 1, 2), verbose=False, test_mode=True)
...
class TestTransactionExecution:
    ...
    def test_execute_multi_move_atomic(self):
        ...
        assert result == ExecuteResult.APPLIED
        assert ledger.get_balance("alice", "USD") == Decimal("8500.0")
        assert ledger.get_balance("alice", "AAPL") == Decimal("10.0")
        assert ledger.get_balance("bob", "USD") == Decimal("1500.0")
        assert ledger.get_balance("bob", "AAPL") == Decimal("90.0")
    ...
    def test_execute_idempotency(self):
        ...
        assert ledger.get_balance("alice", "USD") == Decimal("900.0")  # Only applied once
    ...
    def test_execute_reject_insufficient_funds(self):
        ...
        tx = build_transaction(ledger, [
            Move(Decimal("100.0"), "AAPL", "alice", "bob", "trade")
        ])
        result = ledger.execute(tx)
    ...
    def test_execute_reject_unregistered_unit(self):
        ...
        tx = build_transaction(ledger, [
            Move(Decimal("100.0"), "UNKNOWN", "alice", "bob", "trade")
        ])
        result = ledger.execute(tx)
    ...
    def test_execute_reject_unregistered_wallet(self):
        ...
        tx = build_transaction(ledger, [
            Move(Decimal("100.0"), "USD", "alice", "unknown", "trade")
        ])
        result = ledger.execute(tx)
    ...
    def test_execute_applies_rounding(self):
        ...
        tx = build_transaction(ledger, [
            Move(Decimal("100.456"), "USD", "alice", "bob", "payment")
        ])
        ledger.execute(tx)

        # Rounded to 2 decimal places
        assert ledger.get_balance("bob", "USD") == Decimal("100.46")
...
class TestExecuteContract:
    ...
    def test_execute_with_state_changes(self):
        ...
        new_state = {**old_state, "settled": True, "price": Decimal("150.0")}
...
class TestClone:
    ...
    def test_clone_creates_independent_copy(self):
        ...
        clone = ledger.clone()
        clone.set_balance("alice", "USD", Decimal("500.0"))

        assert ledger.get_balance("alice", "USD") == Decimal("1000.0")
        assert clone.get_balance("alice", "USD") == Decimal("500.0")

    def test_clone_copies_unit_state(self):
        """Clone copies unit state independently."""
        ledger = Ledger("test", verbose=False, test_mode=True)
        ledger.register_unit(_stock("AAPL", "Apple", "treasury"))
        ledger.update_unit_state("AAPL", {"price": Decimal("150.0")})

        clone = ledger.clone()
        clone.update_unit_state("AAPL", {"price": Decimal("200.0")})
...
class TestCloneAt:
    ...
    def test_clone_at_reconstructs_past(self):
        ...
        # Day 2
        ledger.advance_time(datetime(2025, 1, 3))
        tx2 = build_transaction(ledger, [Move(Decimal("200.0"), "USD", "alice", "bob", "p2")])
        ledger.execute(tx2)

        # Clone at day 1
        past_ledger = ledger.clone_at(datetime(2025, 1, 2))

        assert past_ledger.get_balance("alice", "USD") == Decimal("900.0")
        assert past_ledger.get_balance("bob", "USD") == Decimal("100.0")
    ...
    def test_clone_at_can_continue_executing(self):
        ...
        tx_alt = build_transaction(past_ledger, [Move(Decimal("500.0"), "USD", "alice", "bob", "alt")])
        past_ledger.execute(tx_alt)

        # Divergent states
        assert ledger.get_balance("alice", "USD") == Decimal("900.0")
        assert past_ledger.get_balance("alice", "USD") == Decimal("500.0")
...
class TestReplay:
    ...
    def test_replay_recreates_state(self):
        ...
        tx0 = build_transaction(ledger, [Move(Decimal("1000.0"), "USD", "treasury", "alice", "fund")])
        ledger.execute(tx0)
        ...
        assert replayed.get_balance("alice", "USD") == Decimal("900.0")
        assert replayed.get_balance("bob", "USD") == Decimal("100.0")
...
class TestReproducibilityWithDynamicUnits:
    ...
    def test_replay_with_dynamically_created_units(self):
        ...
        assert original_balance == Decimal("100.0")
    ...
    def test_clone_at_before_unit_was_created(self):
        ...
        pending = PendingTransaction(
            moves=(Move(Decimal("1.0"), "DYNAMIC", "system", "alice", "create"),),
            state_changes=(),
        ...
        assert ledger.get_balance("alice", "DYNAMIC") == Decimal("1.0")
    ...
    def test_replay_full_with_dynamic_units(self):
        ...
        tx0 = PendingTransaction(
            moves=(Move(Decimal("10.0"), "UNITX", "system", "alice", "create"),),
            state_changes=(),
        ...
        tx1 = build_transaction(ledger, [Move(Decimal("5.0"), "UNITX", "alice", "bob", "transfer")])
        ledger.execute(tx1)

        # Full replay (from_tx=0) should work correctly
        replayed = ledger.replay(from_tx=0)
        assert "UNITX" in replayed.units
        assert replayed.get_balance("alice", "UNITX") == Decimal("5.0")
        assert replayed.get_balance("bob", "UNITX") == Decimal("5.0")
    ...
    def test_clone_at_with_staggered_unit_creation(self):
        ...
        unit_a = Unit(symbol="UNIT_A", name="Unit A", unit_type="TEST")
        tx1 = PendingTransaction(
            moves=(Move(Decimal("1.0"), "UNIT_A", "system", "alice", "create"),),
            state_changes=(),
        ...
        unit_b = Unit(symbol="UNIT_B", name="Unit B", unit_type="TEST")
        tx2 = PendingTransaction(
            moves=(Move(Decimal("1.0"), "UNIT_B", "system", "alice", "create"),),
            state_changes=(),
        ...
        assert cloned.get_balance("alice", "UNIT_A") == Decimal("1.0")
